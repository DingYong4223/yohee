...
//    }
    ...
    protected void netWorkErrorLogic() {
        ...
        CLog.i("******* Net State Changed. renew player to connect *******" + currentPosition);
    ...
    protected void deleteCacheFileWhenError() {
        clearCurrentCache();
        CLog.i("Link Or mCache Error, Please Try Again " + mOriginUrl);
        if (mCache) {
            CLog.i("mCache Link " + mUrl);
        }
        mUrl = mOriginUrl;
    }
    ...
    public void onInfo(int what, int extra) {
        if (what == MediaPlayer.MEDIA_INFO_BUFFERING_START) {
            ...
            mRotate = extra;
            CLog.i("Video Rotate Info " + extra);
            ...
            mNetInfoModule = new NetInfoModule(getActivityContext().getApplicationContext(), new NetInfoModule.NetChangeListener() {
                ...
                public void changed(String state) {
                    if (!mNetSate.equals(state)) {
                        CLog.i("******* change network state ******* " + state);
                        mNetChanged = true;
                    }
                    mNetSate = state;
                }
