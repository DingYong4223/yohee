...
@Singleton
@WorkerThread
class HistoryDatabase @Inject constructor(application: Application)
    ...
    fun deleteHistory(): Completable = Completable.fromAction {
        database.run {
    ...
    fun deleteHistoryEntry(url: String): Completable = Completable.fromAction {
        database.delete(TABLE_HISTORY, "$KEY_URL = ?", arrayOf(url))
    }

    fun visitHistoryEntry(url: String, title: String?): Completable = Completable.fromAction {
    ...
    fun findHistoryEntriesContaining(query: String): Single<List<HistoryEntry>> =
            Single.fromCallable {
                ...
                return@fromCallable database.query(
                    ...
                    cur.useMap { it.bindToHistoryEntry() }
                }
            }
    ...
    fun getHistoryEntry(url: String): String? =
            database.query(
                ...
                cur.firstOrNullMap { it.getString(0) }
            }
