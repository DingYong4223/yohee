 ...
 */
package com.fula.view.progress
...
class WebProgressBar : View {
    ...
    private val mAlphaInterpolator = LinearInterpolator()
    private val mProgressInterpolator = BezierEaseInterpolator()

    private val mAnimationQueue = ArrayDeque<Animation>()
    ...
    private fun init(context: Context, attrs: AttributeSet) {
        val array = context.theme.obtainStyledAttributes(attrs, R.styleable.WebProgressBar, 0, 0)
        try {
            // Retrieve the style of the progress bar that the user hopefully set
            mProgressColor = array.getColor(R.styleable.WebProgressBar_progressColor, Color.RED)
            mBidirectionalAnimate = array.getBoolean(R.styleable.WebProgressBar_bidirectionalAnimate, false)
            mAnimationDuration = array.getInteger(R.styleable.WebProgressBar_animationDuration, PROGRESS_DURATION)
        } finally {
            array.recycle()
        }
    }

    /**临时显示的进度颜色，加载完成后恢复成原来的颜色 */
    fun setmTempProgressColor(@ColorInt color: Int) {
        if (color != mProgressColor) mProgressColor = color
    }
    ...
    private fun animateView(initialWidth: Int, deltaWidth: Int, maxWidth: Int) {
        val fill = ProgressAnimation(initialWidth, deltaWidth, maxWidth)
