...
public class DefaultItemTouchHelperCallback extends ItemTouchHelper.Callback {
    ...
    public boolean onMove(RecyclerView arg0, RecyclerView.ViewHolder srcHolder, RecyclerView.ViewHolder targetHolder) {
        if (onItemMoveListener != null) {// 回调刷新数据及界面。
            return (isReplace = onItemMoveListener.onItemMove(srcHolder, targetHolder));
        }
        return false;
    }
    ...
    public void onSelectedChanged(RecyclerView.ViewHolder viewHolder, int actionState) {
        ...
        if (null != onItemMoveListener) {
            if (isReplace) {
                isReplace = false;
                onItemMoveListener.onItemMoveEnd(viewHolder);
            }
        }
